...
class Product:
    ...
    def __init__(self, name: str, price: float, stock: int = 0, category: str = "general"):
        ...
        # Increment Product.total_products by 1.
        Product.total_products+=1
        pass

    def __str__(self) -> str:
        # Return "Laptop ($999.99) — 15 in stock"
        return f"{self.name} (${self.price:.2f}) — {self.stock} in stock"
        #return f"Laptop ($999.99) — 15 in stock"


    def __repr__(self) -> str:
        # Return "Product('Laptop', 999.99, stock=15, category='electronics')"
        return f"Product('{self.name}', {self.price}, stock={self.stock}, category='{self.category}')"

        #return f"Product('Laptop', 999.99, stock=15, category='electronics')"
    

    def __eq__(self, other) -> bool:
        # Two products are equal if name AND category match (case-insensitive).
    ...
    def __lt__(self, other) -> bool:
        # Compare by price. Enables sorted(list_of_products).

        return self.price < other.price
        

    def __bool__(self) -> bool:
        # Return True if stock > 0, False if out of stock.
        return self.stock > 0
        

    def __contains__(self, item: str) -> bool:
        # Return True if item (string) is a substring of self.name (case-insensitive).
